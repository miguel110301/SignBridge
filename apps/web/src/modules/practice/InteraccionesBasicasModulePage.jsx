...
import { isInteraccionesBasicasModuleTitle } from './moduleRoutes.js'
import {
...
function resolveInteraccionesBasicasModule(modules) {
  return (
    modules.find((moduleItem) => isInteraccionesBasicasModuleTitle(moduleItem.title))
    ?? null
  )
}
...
export default function InteraccionesBasicasModulePage() {
  const { token } = useAuth()
  ...
  const moduleData = useMemo(() => {
    return resolveInteraccionesBasicasModule(modules)
  }, [modules])
      ...
      <div className="flex min-h-[calc(100dvh-65px)] items-center justify-center bg-[#030712] px-4 text-white">
        <div className="max-w-xl rounded-2xl border border-[#1c2740] bg-[#0a1324] p-6">
          <p className="text-sm text-[#9db0d2]">
            No encontramos el modulo Interacciones Basicas en este momento.
          </p>
          <Link
            to="/practica"
    ...
    <div className="min-h-[calc(100dvh-65px)] bg-[#030712] text-white">
      <MaxWidthWrapper className="py-9 sm:py-12">
        ...
        <p className="mt-3 max-w-3xl text-[1.05rem] leading-relaxed text-[#9db0d2]">
          {moduleData.description || 'Practica guiada de interacciones basicas en LSM.'}
        </p>
